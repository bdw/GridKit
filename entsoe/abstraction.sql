...
-- testing ground for the network abstraction code....
-- TODO;
-- + find connected voltages for stations
-- + create busses for each voltage/station
-- + create links from voltages to station
-- + create transformers between voltage levels
-- + export generators
...
insert into station_transformer (transformer_id, station_id, src_bus_id, dst_bus_id, src_voltage, dst_voltage, src_dc, dst_dc)
     select nextval('line_id'), station_id, src_bus_id, dst_bus_id, src_voltage, dst_voltage, src_dc, dst_dc
...
insert into network_link (link_id, src_bus_id, dst_bus_id, voltage, circuits, dc, underground, under_construction, length_m, tags, geometry)
     select e.line_id, s.network_bus_id, d.network_bus_id, l.voltage, l.circuits, l.dc_line, l.underground, l.under_construction,
       ...
       join station_terminal d on d.station_id = e.station_id[2] and (d.voltage = l.voltage or s.voltage is null and l.voltage is null) and d.dc = l.dc_line;
...
insert into network_transformer (transformer_id, symbol, src_bus_id, dst_bus_id, src_voltage, dst_voltage, src_dc, dst_dc, geometry)
     select t.transformer_id,
       ...
       join topology_nodes n   on t.station_id = n.station_id;
